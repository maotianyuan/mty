...
JS 线程和渲染线程互斥且互相阻塞(同步获取 DOM 信息)，为了避免 GUI 卡顿，通过事件循环在单线程下处理异步任务（不阻塞主线程），引入了浏览器事件循环机制
js 在单线程情况下非阻塞地调用网络、定时器等异步 API
...
- 任务源可以分为 微任务和宏任务
- 微任务包括：Promise.then、MutationObserver（浏览器）、queueMicrotask 等；Node 中还有 process.nextTick
- 宏任务包括：script、setTimeout、setInterval、I/O、UI rendering 等。注意：setImmediate 仅 Node 有；requestAnimationFrame 由浏览器在渲染前调度，一般不归入标准宏任务队列
