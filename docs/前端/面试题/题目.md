...
- 异步
  ...
  - 23.Promise 原理、方法，promise race all allSettled
...
- 类
  - 简写 `class Animal { constructor(public name: string, public age: number) {this.name = name, this.age = age })`
...
- 条件类型
  ...
  - type MyExclude = Exclude<'1' | '2' | '3', '1' | '2' | '4'> 结果为 '3'（排除掉 '1'|'2'，'4' 不在前一个联合中故不参与排除）
  - Extract 抽取
